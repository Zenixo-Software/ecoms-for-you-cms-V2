...
import { categoryCallerAction, getCategoryCallerAction } from "../../redux/categoryRedux/categoryActions";
...
const AddCategory: React.FC<Props> = () => {
  ...
  const closeDrawer = useCallback(
    () => dispatchWindow({ type: "CLOSE_DRAWER" }),
    [dispatchWindow]
  );
  ...
  const validate = (values) => {
    ...
    for (let i = 0; i < subCategory.length; i++) {
      subCat.push({
        title: subCategory[i].name,
        type: localStorage.getItem("shopType").toLowerCase(),
        id: i,
      });
    }
    ...
    axiosInstance
      ...
      .then((response) => {
        dispatch(getCategoryCallerAction((getDataObj.query = search)));
